...
export default function AddClient() {
  ...
  const handleSubmit = async (e) => {
    ...
    let url = 'http://localhost:5000/save_client';
    let method = 'POST';

    if (editingClient) {
      // When editing, ensure mobile_number isn't changed if it's the primary key for the API
      url = `http://localhost:5000/clients/${editingClient.mobile_number}`;
      method = 'PUT';
    }
  ...
  const handleDeleteClient = async (mobileNum) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/clients/${mobileNum}`, { method: 'DELETE' });
      if (!res.ok) {

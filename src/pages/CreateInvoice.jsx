...
export default function CreateInvoice() {
  ...
  const fetchDataAndSetInvoiceNo = async () => {
    try {
      ...
      // Fetch clients
      const clientsRes = await fetch(`${BASE_API_URL}/clients`); 
      if (!clientsRes.ok) throw new Error('Failed to fetch clients');
      const clientsData = await clientsRes.json();
      setClients(clientsData);

      // Fetch invoices to determine the next invoice number
      const invoicesRes = await fetch(`${BASE_API_URL}/invoices`); 
  ...
  const saveInvoice = async () => {
    ...
    try {
      const res = await fetch(`${BASE_API_URL}/save_invoice`, { 
        method: 'POST',
  ...
  return (
    <div className="max-w-screen-lg mx-auto p-6 bg-white rounded-2xl shadow-xl print:shadow-none print:bg-white print:p-0 print:m-0 print:max-w-none print:w-full">
      {/* A4 Print Styling */}
      <style>
        {`
          @media print {
      ...
      <div className="hidden print:block print:w-full print:max-w-[210mm] print:min-h-[297mm] mx-auto">
        <PrintInvoice

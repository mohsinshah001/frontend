...
export default function SavedInvoices() {
    ...
    const fetchInvoices = async () => {
        try {
            ...
            const res = await fetch(`${BASE_API_URL}/invoices`); // URL updated
    ...
    const handleDelete = async (invoiceNo) => {
        // window.confirm() ko remove kiya gaya hai kyunki yeh Canvas environment mein support nahi hota.
        // Asal application mein aapko custom modal use karna hoga confirmation ke liye.
        const confirmDelete = true; // Temporary: Assume confirmed. Implement custom modal for user confirmation.
        if (!confirmDelete) return;

        try {
            const res = await fetch(`${BASE_API_URL}/invoices/${invoiceNo}`, { // URL updated
                method: 'DELETE'
            });
            ...
            // Invoice ko state se remove karein aur message dikhayein (alert ki jagah)
            setInvoices(invoices.filter(inv => inv.invoice_number !== invoiceNo));
            setPaymentMessage('✅ Invoice deleted successfully!'); // Payment message state ko reuse kiya
            setTimeout(() => setPaymentMessage(''), 3000); // 3 seconds baad message clear karein
        } catch (err) {
            console.error('Delete error:', err);
            setError(err.message || 'Failed to delete invoice. Check backend logs.');
            setPaymentMessage('❌ Failed to delete invoice: ' + (err.message || 'Server error.'));
            setTimeout(() => setPaymentMessage(''), 5000); // 5 seconds baad message clear karein
        }
    };

    const handleEdit = (invoice) => {
        // alert() ko remove kiya gaya hai kyunki yeh Canvas environment mein support nahi hota.
        // Asal application mein aapko custom modal ya navigation use karna hoga.
        setPaymentMessage(`💡 Edit functionality for Invoice #${invoice.invoice_number} would go here.`);
    ...
    const handleProcessPayment = async (e) => {
        ...
        if (amount > parseFloat(invoiceToPay.remaining_amount || 0)) {
            setPaymentMessage('⚠️ Payment amount cannot exceed the remaining balance.');
            return;
        }

        try {
            const res = await fetch(`${BASE_API_URL}/invoices/${invoiceToPay.invoice_number}/add_payment`, {
                method: 'POST', // <--- Yahan PUT se POST mein change kiya gaya hai
                headers: { 'Content-Type': 'application/json' },
                // Backend expects 'payment_amount' key
            ...
            // Backend updated invoice object return nahi kar raha, isliye poori list re-fetch karein
            setPaymentMessage(`✅ Payment of ₨ ${amount.toFixed(2)} added successfully to invoice #${invoiceToPay.invoice_number}!`);
            fetchInvoices(); // Re-fetch all invoices to get updated remaining_amount
            handleCancelPayment(); // Close the payment form
            setTimeout(() => setPaymentMessage(''), 3000); // Message clear karein
        } catch (err) {
            console.error('Payment error:', err);
            setPaymentMessage(`❌ Failed to add payment: ${err.message || 'Server error.'}`);
            setTimeout(() => setPaymentMessage(''), 5000); // Message clear karein
        }
    };
                ...
                {invoiceToPay && (
                    <div className="mb-8 p-6 bg-blue-50 rounded-lg shadow-md border border-blue-200">
                        ...
                        <p className="text-gray-700 mb-4">
                            Customer: <strong>{invoiceToPay.customer_name}</strong> | Current Remaining: <strong>₨ {parseFloat(invoiceToPay.remaining_amount).toFixed(2)}</strong>
                        </p>
                                ...
                                {invoices.map((inv) => (
                                    <tr key={inv.invoice_number} className="hover:bg-gray-50 transition duration-150 ease-in-out">
                                        ...
                                        <td className="px-3 py-4 whitespace-nowrap text-sm">
                                            <span className={`font-semibold ${parseFloat(inv.remaining_amount || 0) > 0 ? 'text-orange-600' : 'text-green-700'}`}>
                                                ₨ {inv.remaining_amount ? parseFloat(inv.remaining_amount).toFixed(2) : '0.00'}
                                            </span>
                                        </td>
                                        <td className="px-3 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex flex-col sm:flex-row items-center justify-end gap-1">
                                                ...
                                                {parseFloat(inv.remaining_amount || 0) > 0 && ( // Use remaining_amount here
                                                    <button

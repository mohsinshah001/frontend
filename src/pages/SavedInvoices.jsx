...
const API_BASE_URL = 'https://al-syed-graphics.onrender.com';
...
export default function SavedInvoices() {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        setError(err.message || 'Failed to fetch invoices. Is the backend running and accessible at the deployed URL?');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (invoiceNo) => {
    ...
    try {
      // Use the deployed API_BASE_URL
      const res = await fetch(`${API_BASE_URL}/invoices/${invoiceNo}`, {
        method: 'DELETE'
      });
  ...
  // NEW: Print Functionality Placeholder
  const handlePrint = (invoice) => {
    alert(`🖨️ Print functionality for Invoice #${invoice.invoice_number} would go here.`);
    console.log('Printing Invoice:', invoice);
    // You can navigate to a print-friendly page, open a PDF, or use window.print() here.
  };
  ...
  const handleProcessPayment = async (e) => {
    ...
    try {
      // Use the deployed API_BASE_URL
      const res = await fetch(`${API_BASE_URL}/invoices/${invoiceToPay.invoice_number}/add_payment`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        ...
        try {
          ...
          errorMessage = `${errorMessage} - ${errorText.substring(0, 150)}...`;
        }
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-64px)] bg-red-50 p-6">
        <div className="bg-white p-8 rounded-xl shadow-lg border border-red-300 text-center">
          ...
          <p className="text-gray-700 text-sm">
            Please ensure your backend server is running on `{API_BASE_URL}` and the `/invoices` endpoint is correctly configured.
          </p>
        </div>
      </div>
    );
                    ...
                    <td className="px-3 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex flex-col sm:flex-row items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          className="w-full sm:w-auto bg-blue-500 text-white px-2 py-1 rounded-md text-xs sm:text-sm hover:bg-blue-600 transition duration-150 ease-in-out border border-blue-500"
                        >
                          Print
                        </button>
                        <button
